package com.deshAndDez.ui.screens.reels_video
...
import com.deshAndDez.ui.screens.reels_video.adapter.VideoAdapter
...
@AndroidEntryPoint
class VediosFragment : BaseFragment(R.layout.fragment_vides_player) {

    private lateinit var binding: FragmentVidesPlayerBinding

    val videosAdapter by lazy {
        VideoAdapter(lifecycleScope, onItemClicked = {
...
//                                as? ReelsAdapter.ViewHolder
                    when (currentViewHolder) {
                        ...
                        is VideoAdapter.ViewHolder1 -> {

                        }
                        is VideoAdapter.ViewHolderAds -> {
                            currentViewHolder?.exoPlayer?.playWhenReady = true
                            currentViewHolder?.exoPlayer?.play()
                            currentViewHolder?.binding?.idExoPlayerVIewPause?.isVisible = false
                        }
    ...
    private fun observeFlow() {
        ...
        list.add(
            TutorialVideos(
                2,
                null,
                url = "https://chefshub.site//storage//videos//1692733836.mp4", type = "reels"
            )
        )
        list.add(
            TutorialVideos(
