package com.deshAndDez.ui.screens.reels_photo
...
import com.deshAndDez.ui.screens.reels_photo.adapter.PhotoAdapter
...
@AndroidEntryPoint
class PhotoFragment : BaseFragment(R.layout.fragment_vides_player) {

    private lateinit var binding: FragmentVidesPlayerBinding
//    private val viewModel: TutorialViewModel by activityViewModels()

    val videosAdapter by lazy {
        PhotoAdapter(lifecycleScope, onItemClicked = {
...
//                                as? ReelsAdapter.ViewHolder
    ...
    fun pauseAllPlayers() {
        for (i in 0 until videosAdapter.itemCount) {
            ...
            when (viewHolder) {
                is PhotoAdapter.ViewHolder -> {
                    viewHolder.exoPlayer?.playWhenReady = false
                    viewHolder.exoPlayer?.pause()
                }
                is PhotoAdapter.ViewHolder1 -> {
                }
                is PhotoAdapter.ViewHolderAds -> {
                    viewHolder.exoPlayer?.playWhenReady = false
                    viewHolder.exoPlayer?.pause()
                }
            }
